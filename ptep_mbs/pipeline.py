...
import ptep_mbs.noise
import ptep_mbs.cmb
import ptep_mbs.foregrounds
...
def check_and_fix_config_file(params):
    ...
    try: params.start_mc_num
    except:
        params.start_mc_num = 0
        print_rnk0('WARNING: setting save_coadd=False', rank)
...
class Sims:
    ...
    def __init__(self,paramfile):
        params_spec = importlib.util.spec_from_file_location('params', paramfile)
        params = importlib.util.module_from_spec(params_spec)
        params_spec.loader.exec_module(params)
        params = check_and_fix_config_file(params)
        self.cmb = ptep_mbs.cmb.CMBModel(params)
        self.fg = ptep_mbs.foregrounds.ForegroundModel(params).fg_sims()
        self.noise = ptep_mbs.noise.NoiseModel(params)
    
    def generate_sim(self,idx):
        cmb = self.cmb.cmb_sims(idx)
        noise = self.noise.noise_sims(idx)
        return cmb,self.fg,noise,cmb+self.fg+noise
...
def __main__():
    ...
    if params.make_noise:
        ...
        ptep_mbs.noise.make_noise_sims(params)
    if params.make_cmb:
        print_rnk0('generating cmb simulations', rank)
        ptep_mbs.cmb.make_cmb_sims(params)
    if params.make_fg:
        print_rnk0('generating fg simulations', rank)
        ptep_mbs.foregrounds.make_fg_sims(params)
    if rank==0:
